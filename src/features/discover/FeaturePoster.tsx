...
export default function FeaturePoster({ kind, slug }: { kind: PosterKind; slug?: string }) {
  // A logo dropped into public/features/<slug>.png (or .svg) replaces the drawn
  // poster with no code change. PNG is tried first because that's what the whole
  // shipped set is; the .svg step stays so a future vector version of any single
  // icon can replace its PNG by deleting one file. If neither is there we fall
  // back to the drawing, so a missing file is never a blank card.
  const [artStep, setArtStep] = useState<0 | 1 | 2>(0);
  const artSrc = slug && artStep < 2 ? `/features/${slug}.${artStep === 0 ? "png" : "svg"}` : null;
    ...
    <div className="w-full aspect-[16/10] rounded-[20px] bg-gradient-to-br from-gold-soft to-ivory border border-line overflow-hidden relative">
      {artSrc && (
        <img
          src={artSrc}
          alt=""
          // /features shows all 21 at once; only the few on screen should load.
          loading="lazy"
          decoding="async"
