 ...
 *
 * Court scores come from get_public_session's `matches` (0008), paged one round
 * at a time (‹ / ›) so a long session doesn't scroll forever.
 *
 * FLAG (see PR notes):
...
export default function PublicLivePage() {
  ...
  const [viewedSeq, setViewedSeq] = useState<number | null>(null); // which round's court scores to show
  ...
  // Court scores, paged by round. Default to the latest round with matches.
  const roundSeqs = [...new Set(matches.map((m) => m.roundSequence))].sort((a, b) => a - b);
  const activeSeq = viewedSeq != null && roundSeqs.includes(viewedSeq) ? viewedSeq : roundSeqs[roundSeqs.length - 1] ?? 0;
  const activeIdx = roundSeqs.indexOf(activeSeq);
  const roundMatches = matches.filter((m) => m.roundSequence === activeSeq);
  ...
  return (
    <div className={shell}>
      ...
      {/* Court scores — paged one round at a time */}
      {roundSeqs.length > 0 && (
        <div className="mb-3">
          <div className="flex items-center justify-between mb-2.5">
            <button
              onClick={() => activeIdx > 0 && setViewedSeq(roundSeqs[activeIdx - 1])}
            ...
            <p className="text-gold text-[10px] font-bold uppercase tracking-[0.22em]">
              Round <span className="font-mono tnum">{activeSeq}</span> · Scores
            </p>
            <button
              onClick={() => activeIdx < roundSeqs.length - 1 && setViewedSeq(roundSeqs[activeIdx + 1])}
              disabled={activeIdx >= roundSeqs.length - 1}
          ...
          <div className="space-y-2">
            {roundMatches.map((m, i) => {
              ...
              return (
                <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.04] px-3.5 py-2.5">
                  <p className="text-[9.5px] uppercase tracking-[0.14em] text-ivory/45 mb-1.5">{m.courtName}</p>
                  <div className="flex items-center gap-2">
                    <span className={`flex-1 min-w-0 text-[12.5px] ${aWon ? "text-ivory font-semibold" : "text-ivory/85"}`}>{m.teamA.join(" & ") || "—"}</span>
                    <span className="font-mono tnum text-[15px] font-semibold shrink-0 px-1">
