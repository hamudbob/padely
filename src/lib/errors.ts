 ...
 * quoting PLR-2002 would have cost one message. So every failure the app can
 ...
 * thought of yet:
 *
 *   CURATED  — PLR-1001, PLR-2002 … a known condition with a documented
 *              meaning, likely cause and fix. Listed in CATALOGUE below and
 *              in docs/ERRORS.md.
 *   AUTOMATIC — PLR-U-7F3A … anything unclassified. Derived from the error
...
export function codeFor(error: unknown, where = ""): string {
  ...
  if (pg === "PGRST116") return "PLR-2002"; // single-object request, wrong row count
  if (pg === "23505") return "PLR-9004";
  if (pg === "42501") return "PLR-9003";
  if (pg === "23514") return "PLR-3001"; // check constraint — a rejected format
  if (pg === "P0002") return "PLR-3002"; // our own "not found" convention

  // 2. HTTP status, when the reporting fetch passed one through.
  if (status === 401 || status === 403) return "PLR-1003";
  if (status === 429) return "PLR-9002";
  if (status >= 500) return "PLR-9001";

  // 3. Messages we recognise — ours and GoTrue's.
  if (/email not confirmed/.test(message)) return "PLR-1002";
  if (/invalid login credentials|invalid_grant/.test(message)) return "PLR-1001";
  if (/banned|user is banned/.test(message)) return "PLR-1004";
  if (/not signed in|please (log|sign) in|session expired/.test(message)) return "PLR-1005";
  if (/multiple \(or no\) rows|more than one row/.test(message)) return "PLR-2002";
  if (/could not find your team/.test(message)) return "PLR-2001";
  if (/canvas is unavailable/.test(message)) return "PLR-6002";
  if (/browser can't process images|image/.test(message) && /process|decode|resize/.test(message)) return "PLR-6001";
  if (/failed to fetch|networkerror|load failed|offline/.test(message)) return "PLR-9005";
  if (/session not found/.test(message)) return "PLR-3002";
  if (/match not found/.test(message)) return "PLR-3003";
  if (/finish scoring every match|next round generation/.test(message)) return "PLR-3004";
  if (/round needs at least|no courts are available|nothing before it/.test(message)) return "PLR-3006";
  ...
  const byPlace: Record<string, string> = {
    "PlayPage.getHostHomeSummary": "PLR-2001",
    "ProfilePage": "PLR-2004",
    "LeaguePage": "PLR-4002",
    "TeamDetailPage": "PLR-4001",
    "TeamsPage": "PLR-4001",
    "JoinPage": "PLR-5002",
    "WatchPage": "PLR-5001",
    "HostLivePage.end": "PLR-3005",
    "HostLivePage.nextRound": "PLR-3004",
    "HostLivePage.score": "PLR-3003",
    "HostLivePage.load": "PLR-3002",
    "CreateSessionPage": "PLR-3001",
    "LoginPage": "PLR-1001",
  };
  if (where && byPlace[where]) return byPlace[where];

  // 5. Everything else: a stable key derived from the failure itself.
  return `PLR-U-${hash4(`${where}|${normalise(message)}`)}`;
}
